...
const { data, error, fetchNextPage, hasNextPage, isFetching, isFetchingNextPage, status } = useInfiniteQuery({
    ...
    queryFn: ({ pageParam = 1, limit = 10 }) => triggeredFrom=="user"?getAllChallenges(pageParam, limit,uid):getGroupChallengeList(pageParam, limit,groupId),
  ...
  console.log("32::",challengeList)
    return (
        <>
        {challengeList.length>0&&challengeList.map((challenge, ind) => {
